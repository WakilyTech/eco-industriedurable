        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped" id="datatable">
                ...
                @foreach($listes as $liste)
                    <tr class="gradeX">
                        <td>{{$liste->raison_sociale}}</td>
                        <td>{{$liste->registre_commerce}}</td>
                        <td>{{$liste->matricule_fiscale}}</td>
                        <td>{{$liste->date_creation}}</td>
                        <td>{{$liste->responsable}}</td>
                        <td>{{$liste->capital}}</td>
                        <td >
                            <a class="btn btn-warning waves-effect waves-light" href="{{ route('demande.edit',$liste->id)}}">Consulter</a>
                            /
        ...
        <div class="modal modal-danger fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('demande.destroy','destroy')}}" method="post">
                        {{ method_field('delete') }}
                        {{csrf_field()}}
